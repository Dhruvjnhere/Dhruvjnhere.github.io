...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ── 3D Rotating Favicon (monochrome wireframe globe) ──
  (function animateFavicon() {
    ...
    function draw() {
      ...
      for (let lat = -2; lat <= 2; lat++) {
        const y = cy + lat * 7;
        const w = Math.sqrt(Math.max(0, R * R - (lat * 7) * (lat * 7)));
        ctx.beginPath();
        ctx.moveTo(cx - w, y);
        ctx.lineTo(cx + w, y);
        ctx.stroke();
      ...
      for (let i = 0; i < 6; i++) {
        ...
        ctx.beginPath();
        for (let t = -Math.PI / 2; t <= Math.PI / 2; t += 0.08) {
          const x = cx + Math.sin(t) * R * squeeze;
